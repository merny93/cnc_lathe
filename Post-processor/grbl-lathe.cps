...
properties =
	...
	hasSpeedDial : false,				// true : the spindle is of type Makita RT0700, Dewalt 611 with a Dial to set speeds 1-6. false : other spindle
	machineHomeZ : 0,					// absolute machine coordinates where the machine will move to at the end of the job - first retracting Z, then moving home X Y
	machineHomeX : 0,
	machineHomeY : 0
	};
...
function onOpen()
	...
	myMachine.setTurning(true);
	myMachine.setToolChanger(false);
	myMachine.setNumberOfTools(1);
	myMachine.setNumberOfWorkOffsets(6);
	myMachine.setVendor("Taig");
	myMachine.setModel("Micro lathe");
	myMachine.setControl("GRBL V1.1j");
